...
public class LibroController {
    ...
    public ResponseEntity<String> deleteLibro(@PathVariable Long id) {
        if (libroService.delete(id)) {
            return ResponseEntity.ok("Se ha eliminado correctamente el libro con id: " + id);
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No se ha encontrado el libro con id:" + id);
        }
    }
